...
const getWatchHistory = asyncHandler(async (req, res) => {
    ...
    const skip = (page - 1) * limit;//pagination logic to skip the number of videos to be fetched
    ...
    const user = await User.aggregate([
      { $match: { _id: new mongoose.Types.ObjectId(req.user._id) } },
      {
        $lookup: {
          ...
          as: "watchHistory",
          pipeline: [
            { $skip: skip }, { $limit: Number(limit) },//pagination reduces the number of videos to be fetched
            {
              $lookup: {
                from: "users",
                localField: "owner",
